...
def parse_body(input_stream=sys.stdin.buffer, length=290):
    """
    ...
    try:
        ...
        decode(body)
        logging.info("body len: {}, body {}".format(len(body), body))
        rows = str.split(body, '\n')
        logging.info("rows len: {}, rows {}".format(len(rows), rows))
        if len(rows) < 2:
            return records
...
def config_logging(level=None, filename=None, max_bytes=1024*1024*20, backup_count=5):
    filename = '/Users/lihong/Documents/lh_vector_test.log'
    if level is None:
        level = 'INFO'        
